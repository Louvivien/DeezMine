...
contract Deezmine {
    ...
    event newOwner(address _id, uint256 date, string _ownerNickName);
    event hasBeenStolenOrLost(address _id, uint256 date, string _message);
    event hasBeenRecover(address _id, uint256 date, string _message);
    event warningAlarm(address _id, uint256 date, string _location);
    event historyEvent(address _id, uint256 _date, string _details);
    ...
    function takeOwnership(
        ...
        if (_ownerAdrress != address(0x0)) {
            ...
            numberOfInstrumentOwnerGot[_ownerAdrress] = numberOfInstrumentOwnerGot[_ownerAdrress]
                .add(1);
        ...
        string memory details = append(
            _ownerNickName,
            " is the new owner. mail :",
            _ownerMail
        );
    ...
    function transfer(
        ...
        numberOfInstrumentOwnerGot[msg.sender] = numberOfInstrumentOwnerGot[msg
            .sender]
            .sub(1);
        numberOfInstrumentOwnerGot[_futurOwner] = numberOfInstrumentOwnerGot[_futurOwner]
            .add(1);
        ...
        for (uint256 i = 0; i < ownerOf[msg.sender].length; i++) {
    ...
    function changeOwnerName(
        address _id,
        string memory _newOwnerNickName,
        string memory _newOwnerMail
    ) public isOwner(_id) {
    ...
    function addPictureWithWallet(address _id, string memory _ipfsHash)
        public
        isOwner(_id)
    {
    ...
    function declareStolenOrLost(address _id, string memory _message)
        public
        isOwner(_id)
    {
    ...
    function declareRecover(address _id, string memory _message)
        public
        isOwner(_id)
    {
    ...
    mapping(address => uint256) public numberOfStories;
    ...
    function createStory(address _id, string memory _details)
        public
        isOwner(_id)
    {
